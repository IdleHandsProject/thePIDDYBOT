//TEST PROGRAM FOR TESTING I/O of the PIDDYBOT
...
// LIBRARIES
//For use with board Version 1.1 - ATMEGA32U4 with DRV8835 Motor Driver(small chip)
...
void loop() {
  ...
  Serial.println("Outputting Angle");
  while(1){
...
void testEncoder()
{
  ...
  BE = myEncB.read();
  long LBE = 0;
  Serial.println("Turn Encoder past 100 steps");
  while (BE < 100) {
    BE = myEncB.read();
    if (BE != LBE) {
      Serial.println(BE);
  ...
  Serial.println("Turn encoded back to 0");
  while (BE > 1) {
    BE = myEncB.read();
    if (BE != LBE) {
      Serial.println(BE);
...
void testPots()
{
  ...
  P2 = analogRead(pot2);
  P3 = analogRead(pot3);
  Serial.println("Turn all pots counterclockwise to zero");
  while ((P1 > pT) && (P2 > pT) && (P3 > pT)) {
    P1 = analogRead(pot1);
    P2 = analogRead(pot2);
    P3 = analogRead(pot3);
  ...
  Serial.println("Turn all pots clockwise to full");
  while ((P1 < 950) && (P2 < 950) && (P3 < 950)) {
    P1 = analogRead(pot1);
    P2 = analogRead(pot2);
    P3 = analogRead(pot3);
  }
